...
object StatusDirector : Director() {
    private val playing = listOf(
        ...
        "Third Coil", "Chess", "Nine Men's Morris", "with the space hamster").map { Game.playing(it) }
    private val watching = listOf(
        ...
        "The Path of Lies: A History of the Alliance Military", "the extranet").map { Game.watching(it) }
    private val listening = listOf(
        "skycars zoom by", "Expel 10", "Varrencage", "the hum of the ship", "the hum of a mass relay",
        "the silence of space", "diplomats").map { Game.listening(it) }
