    ...
    override fun onTrigger(event: MessageReceivedEvent, ai: AIResponse) {
        ...
        if (desiredRole != null && selectableRoles.contains(desiredRole)) {
            try {
                ...
                this.log.debug("No roles needed to be removed from $target in ${event.guild}")
            }
            event.guild.controller.addSingleRoleToMember(target, desiredRole).reason("Asked to be a $desiredRoleName").queueAfter(500, TimeUnit.MILLISECONDS) {
